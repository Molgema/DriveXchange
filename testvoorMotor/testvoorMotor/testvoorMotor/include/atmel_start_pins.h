 ...
 * \param[in] pull_mode Pin pull mode
 */
static inline void L_set_pull_mode(const enum port_pull_mode pull_mode)
{
	PORTB_set_pin_pull_mode(3, pull_mode);
}

/**
 * \brief Set L data direction
 *
 * Select if the pin data direction is input, output or disabled.
 * If disabled state is not possible, this function throws an assert.
 *
 ...
 *                      (low power state)
 */
static inline void L_set_dir(const enum port_dir dir)
{
	PORTB_set_pin_dir(3, dir);
}

/**
 * \brief Set L input/sense configuration
 *
 * Enable/disable L digital input buffer and pin change interrupt,
 * select pin interrupt edge/level sensing mode
 *
 * \param[in] isc PORT_ISC_INTDISABLE_gc    = Iterrupt disabled but input buffer enabled
 *                PORT_ISC_BOTHEDGES_gc     = Sense Both Edges
 ...
 *                PORT_ISC_LEVEL_gc         = Sense low Level
 */
static inline void L_set_isc(const PORT_ISC_t isc)
{
	PORTB_pin_set_isc(3, isc);
}

/**
 * \brief Set L inverted mode
 *
 * Enable or disable inverted I/O on a pin
 *
 * \param[in] inverted true  = I/O on L is inverted
 *                     false = I/O on L is not inverted
 */
static inline void L_set_inverted(const bool inverted)
{
 ...
 *                  false = Pin level set to "low" state
 */
static inline void L_set_level(const bool level)
{
	PORTB_set_pin_level(3, level);
}

/**
 * \brief Toggle output level on L
 *
 * Toggle the pin level
 */
static inline void L_toggle_level()
{
	PORTB_toggle_pin_level(3);
}

/**
 * \brief Get level on L
 *
 * Reads the level on a pin
 */
static inline bool L_get_level()
{
